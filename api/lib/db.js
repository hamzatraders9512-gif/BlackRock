...
// `userId` may be stored either as legacy email string or as ObjectId reference to `User`.
// Use Mixed to allow gradual migration while queries resolve both forms.
...
// `userId` may be legacy email string or ObjectId. Keep unique index, but queries
// will attempt both when resolving balances to preserve existing data.
...
/**
 * Helpers to resolve user identifiers (email or ObjectId) and build queries
 * that match either legacy string-stored userId or new ObjectId-stored userId.
 */
async function resolveUserIdentifier(identifier) {
  // Returns { id: ObjectId|null, email: string|null }
  if (!identifier) return { id: null, email: null };

  // If passed a mongoose doc
  if (typeof identifier === 'object' && identifier._id) {
    return { id: identifier._id, email: identifier.email || null };
  }

  // If identifier looks like ObjectId
  if (mongoose.Types.ObjectId.isValid(identifier)) {
    return { id: mongoose.Types.ObjectId(identifier), email: null };
  }

  // Otherwise assume email string; try to resolve to a user record if available
  try {
    const User = mongoose.model('User');
    const u = await User.findOne({ email: identifier }).select('_id email').lean();
    if (u) return { id: u._id, email: u.email };
  } catch (e) {
    // User model may not be registered in some contexts; fall back to returning email
  }

  return { id: null, email: identifier };
}

async function buildUserQuery(identifier) {
  const resolved = await resolveUserIdentifier(identifier);
  if (resolved.id && resolved.email) {
    return { $or: [{ userId: resolved.id }, { userId: resolved.email }] };
  }
  if (resolved.id) return { userId: resolved.id };
  if (resolved.email) return { userId: resolved.email };
...
async function recordTransaction(userId, transactionData) {
  try {
    const resolved = await resolveUserIdentifier(userId);
    const transaction = new TransactionHistory({
      userId: resolved.id || resolved.email,
...
async function getUserBalance(userId) {
  try {
    const q = await buildUserQuery(userId);
    let balance = await UserBalance.findOne(q);

    if (!balance) {
      const resolved = await resolveUserIdentifier(userId);
      balance = new UserBalance({ userId: resolved.id || resolved.email });
      await balance.save();
    }
...
async function getUserTransactions(userId, status = null) {
  try {
    const query = await buildUserQuery(userId);
...
async function recalculateUserBalance(userId) {
  try {
    const q = await buildUserQuery(userId);
    const resolved = await resolveUserIdentifier(userId);
    const balance = await UserBalance.findOne(q) || new UserBalance({ userId: resolved.id || resolved.email });

    // Get all approved transactions (match either ObjectId or legacy email)
    const approved = await TransactionHistory.find({ 
      $or: [ { userId: resolved.id }, { userId: resolved.email } ],
    ...
    // Update balance
    balance.userId = resolved.id || resolved.email;
    ...
    await balance.save();
    console.log(`✓ Balance recalculated for ${resolved.email || resolved.id}: $${balance.currentBalance}`);
    try {
      notify.emit('balance:update', {
        userId: resolved.id || resolved.email,
        currentBalance: balance.currentBalance,
...
async function getBalanceHistory(userId, days = 30) {
  try {
    const q = await buildUserQuery(userId);
    const balance = await UserBalance.findOne(q);
    if (!balance) return [];
...
async function addEarnings(userId, earningAmount, planName = '', sourceTxId = null) {
  try {
    ...
    console.log(`✓ Earnings added for ${resolved.email || resolved.id}: $${earningAmount}`);
    return balance;
  } catch (error) {
  ...
  try {
    ...
    for (const userId of userIds) {
      try {
        ...
        let balance = await UserBalance.findOne({ $or: [{ userId: resolved.id }, { userId: resolved.email }] });
        if (!balance) balance = new UserBalance({ userId: resolved.id || resolved.email });
        ...
        await addEarnings(resolved.id || resolved.email, dailyEarning, 'daily-balance-percent', null);

        // Update lastEarningAt on any plan txs for this user so legacy per-tx jobs skip
        const stringKey = String(resolved.id || resolved.email);
        const userTxs = txs.filter(t => String(t.userId) === stringKey);
...
async function getTransactionStats(userId) {
  try {
    const q = await buildUserQuery(userId);
    const transactions = await TransactionHistory.find(q);
  ...
  try {
    ...
    for (const userId of userIds) {
      try {
        ...
        if (!last) {
          const latestEarn = await TransactionHistory.findOne({ $or: [{ userId: resolved.id }, { userId: resolved.email }], type: 'earnings' }).sort({ submittedAt: -1 }).lean();
        ...
        if (totalEarning > 0) {
          await addEarnings(resolved.id || resolved.email, totalEarning, 'daily-balance-percent');
