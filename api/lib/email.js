...
module.exports = {
    ...
    // Sends a short report to admin when a new user registers
    sendNewUserReport: async function(adminEmail, report) {
        await verifyConnection();
        const { totalUsers = 0, newUserEmail = '', totalApprovedDeposits = 0 } = report || {};
        const mailOptions = {
            from: process.env.GMAIL_USER,
            to: adminEmail,
            subject: `New user registered - Black Rock (${newUserEmail})`,
            html: `
                <div style="font-family: Arial, sans-serif; max-width:600px; margin:0 auto;">
                    <h2 style="color:#333;">New user registration</h2>
                    <p><strong>New user email:</strong> ${newUserEmail}</p>
                    <p><strong>Total users:</strong> ${totalUsers}</p>
                    <p><strong>Total approved deposits (all users):</strong> $${Number(totalApprovedDeposits).toFixed(2)}</p>
                    <hr style="border:none;border-top:1px solid #eee;margin:18px 0;" />
                    <p style="color:#666;font-size:12px;">This is an automated report from Black Rock.</p>
                </div>
            `
        };
        ...
        try {
            await transporter.sendMail(mailOptions);
            console.log(`Admin report sent to ${adminEmail} for new user ${newUserEmail}`);
            return true;
        } catch (error) {
            console.error('Admin report send error:', error);
            return false;
        }
    }
};