...
async function loadPlanAndDeposits(email) {
  try {
    ...
    if (txList) {
      while (txList.firstChild) txList.removeChild(txList.firstChild);
      if (deps.length === 0) {
        ...
        deps.slice(0,8).forEach(d => {
          ...
          meta.textContent = new Date(d.submittedAt).toLocaleString();
          left.appendChild(meta);

          const amountDiv = document.createElement('div');
          amountDiv.className = 'transaction-amount';
          amountDiv.textContent = `$${(d.amount||0).toFixed(2)}`;
