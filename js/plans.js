...
function showEnrollmentConfirmation(planType, planDetails, user) {
  ...
  // build feature list (avoid innerHTML for XSS safety)
  while (modalFeatures.firstChild) modalFeatures.removeChild(modalFeatures.firstChild);
  (planDetails.features || []).forEach(f => {
    const div = document.createElement('div');
    div.className = 'feature-item';
    div.style.fontSize = '0.95rem';

    // create SVG checkmark icon safely
    const svgNS = 'http://www.w3.org/2000/svg';
    const svg = document.createElementNS(svgNS, 'svg');
    svg.setAttribute('class', 'feature-icon');
