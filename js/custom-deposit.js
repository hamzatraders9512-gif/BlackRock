...
function generateQRCode(address, amount) {
  ...
  // Create QR code with address (BEP20 format - just the address)
  // For BEP20 USDT, we use the plain wallet address
  ...
  try {
    ...
    // Fallback: show text representation safely
    ...
    p.textContent = 'Scan address with your wallet';
    ...
    elements.qrCode.appendChild(container);
  }
}
...
async function handleCopyAddress() {
  try {
    ...
    // Visual feedback (avoid innerHTML mutations)
    const originalNodes = Array.from(elements.copyAddressBtnCustom.childNodes).map(n => n.cloneNode(true));
    // Clear children
    while (elements.copyAddressBtnCustom.firstChild) elements.copyAddressBtnCustom.removeChild(elements.copyAddressBtnCustom.firstChild);
    ...
    elements.copyAddressBtnCustom.appendChild(svg);
    elements.copyAddressBtnCustom.appendChild(span);
    
    showToast('Address copied to clipboard', 'success', 2000);
    
    setTimeout(() => {
      while (elements.copyAddressBtnCustom.firstChild) elements.copyAddressBtnCustom.removeChild(elements.copyAddressBtnCustom.firstChild);
      originalNodes.forEach(n => elements.copyAddressBtnCustom.appendChild(n));
    }, 2000);
...
function showFilePreview(file) {
  while (elements.previewContent.firstChild) elements.previewContent.removeChild(elements.previewContent.firstChild);
