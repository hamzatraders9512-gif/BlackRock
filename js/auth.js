...
async function handleFormSubmit(form) {
    ...
    try {
            const response = await fetch('/api/auth/login', {
                ...
                // Guard against empty or non-JSON responses (server may return empty body on error)
                const text = await response.text();
                let data = {};
                if (text && text.length > 0) {
                    try { data = JSON.parse(text); } catch (err) { console.warn('Non-JSON login response', err); }
                }
