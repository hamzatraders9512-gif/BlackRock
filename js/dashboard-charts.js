    ...
    if (balanceChart) {
      ...
      try {
        ...
        const max = Math.max(...vals, 0);
          // Ensure we assign a plain (non-resolver) scales object to avoid Chart warnings
          balanceChart.config = balanceChart.config || {};
          balanceChart.config.options = balanceChart.config.options || {};
          // deep-clone existing scales (if any) to strip resolver proxies
          try {
            const existing = balanceChart.config.options.scales || {};
            const cloned = JSON.parse(JSON.stringify(existing));
            cloned.y = cloned.y || {};
            cloned.y.suggestedMin = Math.floor(min * 0.95);
            cloned.y.suggestedMax = Math.ceil(max * 1.05);
            balanceChart.config.options.scales = cloned;
          } catch (err) {
            // fallback: assign plain object
            balanceChart.config.options.scales = { y: { suggestedMin: Math.floor(min * 0.95), suggestedMax: Math.ceil(max * 1.05) } };
          }
...
async function initCharts() {
  ...
  try {
    ...
    growthEventSource = new EventSource('/api/dashboard/growth-stream', { withCredentials: true });
...
function startBalanceStream() {
  ...
  balanceEventSource = new EventSource('/api/dashboard/balance-stream', { withCredentials: true });
  ...
  balanceEventSource.onerror = (err) => {
    ...
    // Fallback: EventSource failed (often due to cookie/SameSite/CORS issues).
    // Try a fetch-based streaming fallback which supports credentials reliably.
    try {
      startFetchSSEFallback('/api/dashboard/balance-stream');
    } catch (e) {
      // If fallback fails, retry EventSource after a delay.
      setTimeout(startBalanceStream, 3000);
    }
  };
}

// Fetch-based SSE-like fallback: reads the response stream and parses 'data: ' lines.
let _fetchSSEController = null;
function startFetchSSEFallback(url) {
  // cancel existing fallback if present
  if (_fetchSSEController) {
    try { _fetchSSEController.abort(); } catch (e) {}
    _fetchSSEController = null;
  }
  _fetchSSEController = new AbortController();
  const signal = _fetchSSEController.signal;
  ...
  (async () => {
    try {
            ...
            if (line.startsWith('data:')) {
              ...
              try {
                ...
                if (payload && typeof payload === 'object') {
                  // route payloads to the same handlers as EventSource
                  if (payload.balance !== undefined) updateBalanceChart(payload.balance);
                }
              } catch (err) { console.warn('fetchSSE parse error', err); }
            }
          });
        }
      }
      ...
      console.error('fetchSSE error:', err);
      // Retry EventSource after a delay
      setTimeout(startBalanceStream, 3000);
    }
  })();
}
