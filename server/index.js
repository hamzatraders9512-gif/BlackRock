...
// Content-Security-Policy: use per-request nonce for scripts and a stricter policy.
// Note: Frontend inline scripts must be migrated to use the `res.locals.cspNonce` value.
...
const LOCK_TIME_MINUTES = Number(process.env.LOCK_TIME_MINUTES || 30);

// Stricter rate limiters for auth-sensitive endpoints
...
passport.use(new GoogleStrategy({
  ...
  try {
    // Avoid logging full profile in production
    if (process.env.NODE_ENV !== 'production') {
      console.info('Google Profile id:', profile && profile.id);
    }
...
// Middleware
// CORS: allowlist origins via env var `ALLOWED_ORIGINS` (comma-separated).
...
// Admin authorization middleware - restricts access to admin endpoints
function ensureAdmin(req, res, next) {
  if (!req.isAuthenticated || !req.isAuthenticated()) return res.status(401).json({ message: 'Unauthorized' });
  // Require explicit admin role in production. Allow ADMIN_EMAIL only in non-production for dev convenience.
  const isAdminRole = req.user && req.user.role === 'admin';
  const isAdminEmail = req.user && process.env.ADMIN_EMAIL && req.user.email === process.env.ADMIN_EMAIL;
  if (isAdminRole) return next();
  if (process.env.NODE_ENV !== 'production' && isAdminEmail) return next();
...
app.get(['/signup', '/signup.html'], (req, res) => {
  try {
    ...
    if (ref) {
      ...
      html = html.replace(/(<script src="js\/auth-shared.js"><\/script>)/i, `$1\n  <script ${nonce ? `nonce="${nonce}"` : ''}>window.serverReferral = ${JSON.stringify(ref)};<\/script>`);
    }
...
// Middleware: inject per-request CSP nonce into static HTML files before serving.
// This allows inline scripts that have `nonce` attributes to run under the nonce-based CSP.
app.use((req, res, next) => {
  // Only handle GET HTML requests
  ...
  try {
    ...
    // normalize root to index.html if requested
    ...
    const nonce = (res.locals && res.locals.cspNonce) ? res.locals.cspNonce : '';
    if (nonce) {
      // Add nonce to inline <script> tags (those without src)
      html = html.replace(/<script(?![^>]*\bsrc)[^>]*>/gi, `<script nonce="${nonce}">`);
      // Optionally add nonce to external scripts too (safe)
      html = html.replace(/<script([^>]*\bsrc[^>]*)>/gi, (m, attrs) => {
    ...
    console.error('Error injecting CSP nonce for', req.url, e);
    return next();
  }
});
...
app.get('/vendor/chart.umd.min.js', (req, res) => {
  const fs = require('fs');
  const localPath = path.join(__dirname, '../vendor/chart.umd.min.js');
  const fallbackLocal = path.join(__dirname, '../js/chart.js');

  // Serve local vendor file if present
  if (fs.existsSync(localPath)) {
    return res.sendFile(localPath);
  }

  // Fallback to bundled js/chart.js if available in repo (avoids CDN in dev)
  if (fs.existsSync(fallbackLocal)) {
    res.setHeader('Content-Type', 'application/javascript');
    return res.sendFile(fallbackLocal);
  }

  // Otherwise proxy from CDN as before
  const https = require('https');
...
app.get('/vendor/chartjs-plugin-datalabels.min.js', (req, res) => {
  const fs = require('fs');
  const localPath = path.join(__dirname, '../vendor/chartjs-plugin-datalabels.min.js');

  // Serve local vendor plugin if present
  if (fs.existsSync(localPath)) {
    res.setHeader('Content-Type', 'application/javascript');
    return res.sendFile(localPath);
  }
...
const { generateOTP, sendOTPEmail, sendResetPasswordEmail, sendNewUserReport } = require('../api/lib/email');
...
app.post('/api/auth/signup', async (req, res) => {
  try {
    ...
    // Prepare admin report (send asynchronously; do not fail signup on email error)
    (async () => {
      try {
        ...
        try {
          ...
          const agg = await UserBalance.aggregate([{ $group: { _id: null, total: { $sum: '$totalDeposits' } } }]);
          totalApprovedDeposits = (agg && agg[0] && agg[0].total) ? agg[0].total : 0;
        } catch (e) {
          console.warn('Could not compute totalApprovedDeposits:', e);
        }

        const adminEmail = process.env.ADMIN_EMAIL;
        if (adminEmail) {
          await sendNewUserReport(adminEmail, {
            totalUsers,
            newUserEmail: email,
            totalApprovedDeposits
          });
        } else {
          console.warn('ADMIN_EMAIL not configured; skipping admin new-user report');
        }
      } catch (e) {
        console.error('Failed to send admin new-user report:', e);
      }
    })();
...
app.post('/api/auth/verify-otp', async (req, res) => {
  try {
    ...
    console.info('[OTP VERIFY] attempt for email:', maskEmail(email));
    ...
    console.info('[OTP VERIFY] User found for email:', maskEmail(email), 'isEmailVerified:', user.isEmailVerified);
...
app.post('/api/auth/login', loginLimiter, async (req, res) => {
  try {
    ...
    if (user.lockUntil && user.lockUntil.getTime && user.lockUntil.getTime() > Date.now()) {
      console.warn('Login attempt for locked account:', maskEmail(email));
      return res.status(423).json({ message: `Account locked. Try again after ${new Date(user.lockUntil).toISOString()}` });
    }
    ...
    if (!isValidPassword) {
      ...
      try {
        ...
        if (user.failedLoginAttempts >= MAX_FAILED_LOGIN) {
          ...
          await user.save();
          console.warn('User account locked due to failed logins:', maskEmail(email));
          return res.status(423).json({ message: 'Account locked due to too many failed login attempts. Try again later.' });
        }
        await user.save();
      } catch (e) {
        console.error('Failed to update failed login attempts:', e);
      }
      return res.status(400).json({ message: 'Invalid email or password' });
    }
...
app.get('/api/affiliate/info', async (req, res) => {
  try {
    ...
    // compute stage and reward using affiliate tiers:
    // <10 => Silver ($5), >=10 and <25 => Gold ($7), >=25 => Platinum ($10)
    const count = Number(user.referralsCount || 0);
    let stage = 'Silver'; let reward = 5;
    if (count >= 25) { stage = 'Platinum'; reward = 10; }
    else if (count >= 10) { stage = 'Gold'; reward = 7; }

    // fetch recent referral-related transactions for this user
    const txs = await db.TransactionHistory.find({ $or: [{ userId: user._id }, { userId: user.email }], type: 'earnings', 'details.planName': 'referral' })
    ...
    // Enrich transactions with user-friendly fields so frontend can render name, status and reward
    const recentReferrals = await Promise.all(txs.map(async (t) => {
      let userLabel = '—';
      try {
        // `t.userId` may be an ObjectId or email string
        if (t.userId) {
          const uid = String(t.userId || '');
          if (mongoose.Types.ObjectId.isValid(uid)) {
            const u = await User.findById(uid).select('firstName lastName email').lean();
            if (u) userLabel = ((u.firstName || '') + (u.lastName ? (' ' + u.lastName) : '')).trim() || u.email || '—';
          } else {
            const u = await User.findOne({ email: uid }).select('firstName lastName email').lean();
            if (u) userLabel = ((u.firstName || '') + (u.lastName ? (' ' + u.lastName) : '')).trim() || u.email || uid;
            else userLabel = uid;
          }
        }
      } catch (e) {
        userLabel = (t.userId && String(t.userId)) || '—';
      }
      return {
        transactionId: t._id,
        user: userLabel,
        amount: t.amount,
        description: t.description || (t.details && t.details.planName) || '',
    ...
    res.json({
      success: true,
      referralCode: user.referralCode,
      // direct referral link should open the signup page so clicks lead to registration flow
      ...
      reward,
      recentReferrals,
      // include some user profile info so frontend can show names and avatar without additional calls
      user: {
...
app.post('/api/affiliate/register', async (req, res) => {
  try {
    ...
    // determine reward per referral based on new count (after adding this referral)
    const c = referrer.referralsCount;
    let reward = 5;
    if (c >= 25) reward = 10;
    else if (c >= 10) reward = 7;
    else reward = 5;

    // credit both users via db.addEarnings using user _id for new writes
    try {
      await db.addEarnings(referrer._id, reward, 'referral', null);
    } catch (e) { console.error('Failed to credit referrer', e); }
    try {
      await db.addEarnings(referred._id, reward, 'referral', null);
    } catch (e) { console.error('Failed to credit referred user', e); }

    // update referral reward totals on both users for quick display
    referrer.totalReferralRewards = (referrer.totalReferralRewards || 0) + reward;
    await referrer.save();
    try {
      referred.totalReferralRewards = (referred.totalReferralRewards || 0) + reward;
      await referred.save();
    } catch (e) { console.error('Failed to update referred totals', e); }

    // Emit affiliate update so SSE clients receive real-time changes
    try {
      const txs = await db.TransactionHistory.find({ $or: [{ userId: referrer._id }, { userId: referrer.email }], type: 'earnings', 'details.planName': 'referral' })
        .sort({ submittedAt: -1 }).limit(8).lean();
      const recentReferrals = await Promise.all((txs || []).map(async (t) => {
        let userLabel = '—';
        try {
        ...
        } catch (e) { userLabel = (t.userId && String(t.userId)) || '—'; }
        return {
          transactionId: t._id,
          user: userLabel,
          amount: t.amount,
      ...
      notify.emit('affiliate:update', {
        userId: referrer._id,
        referralCode: referrer.referralCode,
        referralLink: `${req.protocol}://${req.get('host')}/signup.html?ref=${referrer.referralCode}`,
        referralsCount: referrer.referralsCount || 0,
        totalReferralRewards: referrer.totalReferralRewards || 0,
        recentReferrals
      });
    } catch (emitErr) { console.error('Failed to emit affiliate update', emitErr); }
...
// SSE endpoint for affiliate realtime updates: GET /api/affiliate/stream
app.get('/api/affiliate/stream', (req, res) => {
  try {
    if (!req.isAuthenticated()) return res.status(401).json({ message: 'Not authenticated' });
    ...
    const onUpdate = (payload) => {
      try {
        if (!payload) return;
        const payloadId = payload.userId;
        const match = (String(payloadId) === String(userId)) || (payloadId === req.user.email);
        if (!match) return;
        const data = JSON.stringify(payload);
        res.write(`event: affiliate_update\n`);
        res.write(`data: ${data}\n\n`);
      } catch (e) {
        console.error('SSE affiliate send error', e);
      }
    };

    notify.on('affiliate:update', onUpdate);

    // Send initial affiliate snapshot
    (async () => {
      try {
        const user = await User.findById(req.user.id).select('-password -verificationOTP -resetPasswordToken');
        if (!user) return;
        ...
        if (count >= 25) { stage = 'Platinum'; reward = 10; }
          ...
          // fetch recent referral earnings and enrich for frontend
          const txs = await db.TransactionHistory.find({ $or: [{ userId: user._id }, { userId: user.email }], type: 'earnings', 'details.planName': 'referral' })
            .sort({ submittedAt: -1 }).limit(8).lean();
          const recentReferrals = await Promise.all((txs || []).map(async (t) => {
            let userLabel = '—';
        ...
        res.write(`event: affiliate_init\n`);
        res.write(`data: ${JSON.stringify(payload)}\n\n`);
      } catch (e) {
        console.error('Failed to send affiliate_init', e);
      }
    })();

    req.on('close', () => {
      notify.removeListener('affiliate:update', onUpdate);
      try { res.end(); } catch (e) {}
    });
  } catch (err) {
    console.error('Affiliate stream error', err);
...
// Test / debugging endpoints removed for production hardening.

// Deposit Schema
// `userId` can be legacy email string or ObjectId for new records
...
app.post('/api/deposits/submit', async (req, res) => {
  try {
    ...
    await db.recordTransaction(req.user._id, {
      type: 'deposit',
      amount: getPlanAmount(planType),
...
app.get('/api/deposits/my', async (req, res) => {
  try {
    ...
    const deposits = await Deposit.find({ $or: [{ userId: req.user._id }, { userId: req.user.email }] })
      .sort({ submittedAt: -1 });
...
app.get('/api/plans/my', async (req, res) => {
  try {
    ...
    const activePlans = await PlanEnrollment.find({ 
      $or: [{ userId: req.user._id }, { userId: req.user.email }],
      status: 'active'
    }).populate('depositId');
...
app.post('/api/deposits/custom-submit', async (req, res) => {
  try {
    // Minimal logging: avoid printing headers, session IDs, or full user objects
    console.info('Custom deposit request received', {
      authenticated: !!(req.isAuthenticated && req.isAuthenticated()),
      user: req.user ? maskEmail(req.user.email) : null
    });
    ...
    const recordedTx = await db.recordTransaction(user._id, {
      type: 'deposit',
      amount: amount,
...
app.post('/api/auth/reset-password', resetLimiter, async (req, res) => {
  try {
    ...
    user.resetPasswordToken = undefined;
    user.resetPasswordExpires = undefined;
    // Reset failed login attempts and unlock account after password reset
    try {
      user.failedLoginAttempts = 0;
      user.lockUntil = undefined;
    } catch (e) { console.warn('Failed to reset lock fields on password reset', e); }
    await user.save();
...
app.get('/api/dashboard/balance-stream', (req, res) => {
  console.log('[SSE] incoming /api/dashboard/balance-stream request from', req.ip, 'authenticated=', req.isAuthenticated());
  if (!req.isAuthenticated()) {
    console.log('[SSE] /api/dashboard/balance-stream unauthorized:', req.ip);
    return res.status(401).json({ message: 'Unauthorized' });
  }
  ...
  // CORS handled by allowlist middleware. Vary on Origin for caches.
...
app.get('/api/dashboard/balance', async (req, res) => {
  try {
    ...
    const balanceSummary = await db.getBalanceSummary(req.user._id);
    const balance = balanceSummary.currentBalance || 0;
...
app.get('/api/dashboard/growth', async (req, res) => {
  try {
    ...
    if (!hasEarnings) {
      const enrollments = await PlanEnrollment.find({ $or: [{ userId: req.user._id }, { userId: req.user.email }], status: 'active' }).populate('depositId');
...
app.get('/api/dashboard/growth-stream', (req, res) => {
  console.log('[SSE] incoming /api/dashboard/growth-stream request from', req.ip, 'authenticated=', req.isAuthenticated());
  if (!req.isAuthenticated()) {
    console.log('[SSE] /api/dashboard/growth-stream unauthorized:', req.ip);
    return res.status(401).json({ message: 'Unauthorized' });
  }
  const userId = req.user._id;
  res.setHeader('Content-Type', 'text/event-stream');
...
app.get('/api/dashboard/balance-history', async (req, res) => {
  try {
    ...
    if (hours && hours > 0) {
      const history = await db.getBalanceHistoryHours(req.user._id, hours);
    ...
    const history = await db.getBalanceHistory(req.user._id, days);
...
app.get('/api/withdrawals/my', async (req, res) => {
  try {
    ...
    const withdrawals = await Withdrawal.find({ $or: [{ userId: req.user._id }, { userId: req.user.email }] })
      .sort({ submittedAt: -1 });
...
app.get('/api/admin/pending-transactions', ensureAdmin, async (req, res) => {
  try {
    if (!req.isAuthenticated()) return res.status(401).json({ message: 'Unauthorized' });
...
app.get('/api/transactions/pending', ensureAdmin, async (req, res) => {
  try {
    if (!req.isAuthenticated()) {
      return res.status(401).json({ message: 'Unauthorized' });
    }
...
app.post('/api/transactions/:transactionId/approve', ensureAdmin, async (req, res) => {
  try {
    if (!req.isAuthenticated()) {
      return res.status(401).json({ message: 'Unauthorized' });
    }
...
// Admin debug route: only enabled in non-production environments
if (process.env.NODE_ENV !== 'production') {
  app.post('/api/admin/debug/process-earnings', ensureAdmin, async (req, res) => {
    try {
      if (!req.isAuthenticated()) return res.status(401).json({ message: 'Unauthorized' });
...
app.post('/api/transactions/:transactionId/reject', ensureAdmin, async (req, res) => {
  try {
    if (!req.isAuthenticated()) {
      return res.status(401).json({ message: 'Unauthorized' });
    }
...
// Approve a deposit
app.post('/api/admin/deposits/:depositId/approve', ensureAdmin, async (req, res) => {
  try {
    if (!req.isAuthenticated()) return res.status(401).json({ message: 'Unauthorized' });
    ...
    console.info('Deposit approved', { id: deposit._id.toString(), approvedBy: maskEmail(req.user && req.user.email) });
...
// Reject a deposit
app.post('/api/admin/deposits/:depositId/reject', ensureAdmin, async (req, res) => {
  try {
    if (!req.isAuthenticated()) return res.status(401).json({ message: 'Unauthorized' });
    // TODO: Add admin role check
    ...
    console.info('Deposit rejected', { id: deposit._id.toString(), rejectedBy: maskEmail(req.user && req.user.email) });
...
app.post('/api/admin/plans/:planId/approve', ensureAdmin, async (req, res) => {
  try {
    if (!req.isAuthenticated()) return res.status(401).json({ message: 'Unauthorized' });
    // TODO: Add admin role check
    ...
    console.info('Plan enrollment approved', { id: plan._id.toString(), approvedBy: maskEmail(req.user && req.user.email) });
...
app.post('/api/admin/plans/:planId/reject', ensureAdmin, async (req, res) => {
  try {
    if (!req.isAuthenticated()) return res.status(401).json({ message: 'Unauthorized' });
    // TODO: Add admin role check
    ...
    console.info('Plan enrollment rejected', { id: plan._id.toString(), rejectedBy: maskEmail(req.user && req.user.email) });
...
app.post('/api/admin/withdrawals/:withdrawalId/approve', ensureAdmin, async (req, res) => {
  try {
    if (!req.isAuthenticated()) return res.status(401).json({ message: 'Unauthorized' });
    // TODO: Add admin role check
    ...
    console.info('Withdrawal approved', { id: withdrawal._id.toString(), approvedBy: maskEmail(req.user && req.user.email) });
...
// Reject a withdrawal
app.post('/api/admin/withdrawals/:withdrawalId/reject', ensureAdmin, async (req, res) => {
  try {
    if (!req.isAuthenticated()) return res.status(401).json({ message: 'Unauthorized' });
    // TODO: Add admin role check
    ...
    console.info('Withdrawal rejected', { id: withdrawal._id.toString(), rejectedBy: maskEmail(req.user && req.user.email) });
...
app.get('/api/transactions/my', async (req, res) => {
  try {
    // Minimal logging: do not print full user/session objects
    console.info('/api/transactions/my requested', { authenticated: !!(req.isAuthenticated && req.isAuthenticated()), user: req.user ? maskEmail(req.user.email) : null });
    ...
    // Use user._id (ObjectId) instead of email for consistent queries
    const userId = req.user._id;
